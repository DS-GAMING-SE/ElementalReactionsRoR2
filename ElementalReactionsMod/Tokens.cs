...
namespace ElementalReactionsMod
{
    public static class Tokens
    {
        ...
        public static void Initialize()
        {
            ...
            LanguageAPI.Add($"{prefix}REACTION_SUPERCONDUCT_DESCRIPTION", $"{CryoText()} + {ElectroText()}. Create a blast of ice dealing {DamageValueText(superconductDamageCoefficient)} and increasing all non-elemental damage dealt by {DamageMultiplierText(superconductDamageMultiplier + 1f)} for {UtilityText($"{superconductDuration}s")}.");
            ...
            string stellarLinchpinDescSwirl = $"{AnemoText("Stellar Swirl")}: Create a Stellar Vortex which explodes after {UtilityText(stellarSwirlMaxDuration+"s")} for {DamageText($"{stellarSwirlMinDamage * 100f}%")} {CryoText()} {DamageText("damage")}. Continuously triggering {AnemoText("Stellar Swirl")} will upgrade the vortex, dealing {DamageText((stellarSwirlMaxDamage * 100f)+"%")} in a larger radius.";
        ...
        {
            return $"Activating your {UtilityText("Special skill")} will activate the Delusion for {UtilityText(delusionDuration+"s")}. While active, {RedText($"healing received is reduced by {delusionHealingReceivedReduction * 100f}%")} {StackingText($"(+{delusionHealingReceivedReduction * 100f}% per stack)")} and damaging enemies with any {UtilityText("skill")} will periodically fire {DamageText("attacks of ")+element}, dealing {DamageText($"{delusionDamageCoefficient * 100f}% base damage")} {StackingText($"(+{delusionStackDamageCoefficient * 100f}% per same-element stack)")} at the cost of {RedText(delusionHealthPercentCost * 100f + "%")} {StackingText($"(+{delusionHealthPercentCost * 100f + "%"} per same-element stack)")} of your {RedText("current health")}. {StackingText("A separate attack will be fired for each Delusion you have of a unique element")}.";
        }
        ...
        {
            return $"<style=cIsDamage>{(value) * 100}%-{(value2) * 100}%</style>";
        }
